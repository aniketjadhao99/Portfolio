...
def process_file():
    ...
    # regex for headers that MUST BE on their own line (or almost)
    ...
    for slip_num, content in slips_data:
        ...
        if q_markers:
            ...
            # Fallback split: Use large whitespace gaps as question dividers (3+ newlines)
            parts = re.split(r'\n\s*\n\s*\n', content)
    ...
    print(f"Update complete (v11). Processed {len(slips_data)} slips.")
