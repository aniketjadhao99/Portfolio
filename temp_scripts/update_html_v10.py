...
def process_file():
    ...
    for slip_num, content in slips_data:
        ...
        if q_markers:
            ...
            # Fallback split: Use large whitespace gaps as question dividers
            # If a slip has no Q markers, it usually has two parts separated by 3+ newlines
            parts = re.split(r'\n\s*\n\s*\n\s*\n', content)
        ...
        for q in questions:
            ...
            if not re.search(header_pattern, q_content):
                blocks.append({'label': None, 'code': q_content})
            else:
        ...
        for q in processed_questions:
            q_num = q['label'][1:] if q['label'].startswith('Q') else q['label']
    ...
    print(f"Update complete (v10). Processed {len(slips_data)} slips.")
