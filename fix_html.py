...
def extract_code_block(text):
    """Extract complete code block from question text"""
    lines = text.split('\n')
    code_lines = []
    capture = False
    
    for line in lines:
        # Start capturing if line contains import, class, public, <%
        if line.strip().startswith(('import ', 'class ', 'public ', '<%', '<html')):
            capture = True
        
        if capture:
            code_lines.append(line)
            # Stop capturing if we hit certain markers (next question, end of slip)
            if line.strip().startswith(('2.', '2 .', 'Slip', '<%=')) and code_lines and len(code_lines) > 5:
                if not line.strip().startswith(('<%', '<html')):
                    code_lines.pop()
                    break
...
for i in range(1, len(slips), 2):
    ...
    # Extract questions - split by "1.", "2.", etc.
    question_pattern = r'(\d+)\.\s+'
    questions = re.split(question_pattern, slip_content)
  ...
  <div class="slip-body">'''
    ...
    for j in range(2, len(questions), 2):
        question_text = questions[j] if j < len(questions) else ""
        
        slip_html += f'<div class="q-label">Question {q_num}</div>'
        
        # Extract code from question
        code = extract_code_block(question_text)
        
        if code and (code.startswith('import ') or code.startswith('class ') or 
                    code.startswith('public ') or code.startswith('<%')):
    ...
    slip_html += '</div></div>\n'
    html_content += slip_html

final_html = html_content + html_footer
